...
//
// Navigate here from the Track button in RideCard (Upcoming tab).
//
// ── Simple push (no GoRouter) ─────────────────────────────────────
//   Navigator.of(context).push(TrackPassengerPage.route(trackingRide));
//
// ── With GoRouter ─────────────────────────────────────────────────
...
class TrackPassengerPage extends StatefulWidget {
  ...
  /// Convenience static route for Navigator.push usage
      ...
      child: Stack(
        children: [
          ...
          Align(
            ...
            child: _CircleMarker(color: AppColors.primaryPurple, icon: Icons.navigation_rounded),
          ),
          Align(
            alignment: const Alignment(0.55, 0.30),
            child: _LabeledMarker(color: AppColors.success, icon: Icons.location_on_rounded, label: 'Pickup'),
          ),
          if (status.showDropoffMarker)
            Align(
              alignment: const Alignment(-0.45, -0.55),
              child: _LabeledMarker(color: AppColors.primaryPurple, icon: Icons.flag_rounded, label: 'Drop-off'),
            ),
        ],
      ),
    );
...
class _LabeledMarker extends StatelessWidget {
  ...
  final IconData icon;
  final String label;
  const _LabeledMarker({required this.color, required this.icon, required this.label});
...
class _GridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final p = Paint()..color = const Color(0xFFCDD7E2)..strokeWidth = 1;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawPath(
      Path()
        ...
        ..quadraticBezierTo(size.width * 0.54, size.height * 0.52, size.width * 0.64, size.height * 0.62),
      p,
    );
  }
  @override
